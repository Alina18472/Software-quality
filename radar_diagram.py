...
class RadarDiagram():
    ...
    def draw(self, data, labels, title):
        ...
        axs.set_rgrids((np.min(all_data), (np.min(all_data) + np.max(all_data))/2, 
                       np.max(all_data)*3/4, np.max(all_data)))

        for ind, dataset in enumerate(data):
            axs.plot(theta, dataset, color=colors[ind])
            
        axs.set_varlabels(spoke_labels)

      
        legend_labels = ('Начальные вычисления', 'Вычисления через 1/4 проходов', 
                        'Вычисления через 3/4 проходов', 'Конечные вычисления')
        axs.legend(legend_labels, loc=(-.13, .99), labelspacing=0.1, fontsize='medium')
